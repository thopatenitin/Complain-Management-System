...
function makeComplain()
{
	...
	header("Location: view.php?mod=Civilean&view=compDetails");	
	exit;	
}
...
{
	//echo 'Make Complain...';
    $corporatorname = $_POST['corporatorname'];
	$Password = $_POST['Password'];
	$Address = $_POST['Address'];
	$Email = $_POST['Email'];
	$Mobile = $_POST['Mobile'];
	
	$sql = "INSERT INTO tbl_corporator (ename, epass, address, email, e_mobile, date_time) 
			VALUES ('$corporatorname', '$Password', '$Address', '$Email', '$Mobile', NOW())";
		
	$result = dbQuery($sql);
	//header("Location: index.php?view=bal&error=" . urlencode("$data"));	
	header("Location: view.php?mod=Administrator&view=enggDetails");	
	exit;	
}
...
function addCust()
{
	...
	$sql = "INSERT INTO tbl_Civilean (cname, cpass, address, email, c_mobile, date_time) 
			VALUES ('$CustomerName', '$Password', '$Address', '$Email', '$Mobile', NOW())";
		
	$result = dbQuery($sql);
	//header("Location: index.php?view=bal&error=" . urlencode("$data"));	
	header("Location: view.php?mod=Administrator&view=custDetails");	
	exit;	
}
...
function assignComplain()
{
	...
	$sql = "SELECT ename FROM tbl_corporator WHERE eid = $engId";	
	$result = dbQuery($sql);
	...
	//	echo $sql;		
	$result = dbQuery($sql);
	header("Location: view.php?mod=Administrator&view=compDetails");	
	exit;	
}
...
function commentOnComplain()
{
	...
	$sql = "UPDATE tbl_complains 
			...
			WHERE cid = $compId";	
	$result = dbQuery($sql);
	header("Location: view.php?mod=corporator&view=viewComplain");	
	exit;	
}
...
function deleteEngg()
{
    ...
    $eId = $_GET['eId'];
	//$empComment = $_POST['empComment'];
	$sql = "DELETE FROM tbl_corporator 
				WHERE eid = $eId";	
	$result = dbQuery($sql);
	header("Location: view.php?mod=Administrator&view=enggDetails");	
	exit;	
...
function editCust()
{
	//echo 'Make Complain...';
    $cid = $_POST['cid'];
	$CivileanName = $_POST['CivileanName'];
	$Password = $_POST['Password'];
	$Address = $_POST['Address'];
	$Email = $_POST['Email'];
	$Mobile = $_POST['Mobile'];
	
	$sql = "UPDATE tbl_Civilean 
			SET cname = '$CivileanName', 
				cpass = '$Password', 
				address = '$Address', 
				email = '$Email', 
				c_mobile = '$Mobile'
...
function selectPlan(){
	...
	if(dbNumRows($res)==1){
		header("Location: view.php?mod=Civilean&view=planExist");
	}else {
		$sql = "INSERT INTO tbl_plans (cid, plans, amt, plan_date)
				VALUES ($userId,'$plan_string',$amt,'$date')";
		dbQuery($sql);
		header("Location: view.php?mod=Civilean&view=planSuccess");
	}
}
